...
app.get("/decode", async (req, res) => {
  ...
  if (!code) {
    res.status(400).json({ error: "Please provide a code" });
  } else {
    ...
    try {
      ...
      res.status(200).json({ code, decoded, 'schemeUsed':schemeName });
    } catch (err) {
...
app.get("/encode", async (req, res) => {
  ...
  if (!scheme || !str) {
    res
      .status(400)
      .json({ error: "Please provide a string and a coding scheme" });
  } else {
    if (!validSchemes.includes(scheme)) {
      res.status(400).json({ error: "Please provide a valid scheme" });
    } else {
      ...
      try {
        ...
        res.status(200).json({ str, encoded, scheme });
      } catch (err) {
        res.status(400).json(err.message);
      }
    }
